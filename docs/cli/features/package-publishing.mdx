...
A nono pack is a signed bundle of artifacts — profiles, hooks, plugins, instructions, scripts, trust policies, groups — distributed via the registry and verified with Sigstore at install time. Packs are the recommended way to ship anything that used to be a built-in profile or a hand-installed agent integration.
...
- `groups` — additional capability groups, prefixed to avoid collision with built-ins
...
- `groups` artifacts need `prefix` — must namespace your group names so they don't clash with built-ins.
...
### Profiles That Replace a Former Built-in

If your pack ships a profile with an `install_as` name that used to be compiled into the CLI (e.g. `claude-code` was built-in until v0.43), you also need to register it in the migration table so users on older versions get a friendly auto-pull prompt instead of a "profile not found" error. See [PACK_PROVIDED_PROFILES](#registering-a-pack-provided-profile) below.
